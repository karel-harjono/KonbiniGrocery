DROP TABLE IF EXISTS review;
DROP TABLE IF EXISTS shipment;
DROP TABLE IF EXISTS productinventory;
DROP TABLE IF EXISTS warehouse;
DROP TABLE IF EXISTS orderproduct;
DROP TABLE IF EXISTS incart;
DROP TABLE IF EXISTS product;
DROP TABLE IF EXISTS category;
DROP TABLE IF EXISTS ordersummary;
DROP TABLE IF EXISTS paymentmethod;
DROP TABLE IF EXISTS customer;


CREATE TABLE customer (
    customerId          INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE paymentmethod (
    paymentMethodId     INT IDENTITY,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE ordersummary (
    orderId             INT IDENTITY,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE category (
    categoryId          INT IDENTITY,
    categoryName        VARCHAR(50),    
    PRIMARY KEY (categoryId)
);

CREATE TABLE product (
    productId           INT IDENTITY,
    productName         VARCHAR(1000),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        VARBINARY(MAX),
    productDesc         VARCHAR(2000),
    categoryId          INT,
    PRIMARY KEY (productId),
    FOREIGN KEY (categoryId) REFERENCES category(categoryId)
);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE warehouse (
    warehouseId         INT IDENTITY,
    warehouseName       VARCHAR(30),    
    PRIMARY KEY (warehouseId)
);

CREATE TABLE shipment (
    shipmentId          INT IDENTITY,
    shipmentDate        DATETIME,   
    shipmentDesc        VARCHAR(100),   
    warehouseId         INT, 
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory ( 
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (productId, warehouseId),   
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE review (
    reviewId            INT IDENTITY,
    reviewRating        INT,
    reviewDate          DATETIME,   
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),          
    PRIMARY KEY (reviewId),
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

INSERT INTO category(categoryName) VALUES ('Anime Movies');
INSERT INTO category(categoryName) VALUES ('Games');
INSERT INTO category(categoryName) VALUES ('Model Kits');
INSERT INTO category(categoryName) VALUES ('Snacks');
INSERT INTO category(categoryName) VALUES ('Stationaries');

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Your name', 1, 'The movie Your name on Blu-ray', 34.98);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Demon Slayer - Mugen Train', 1, 'Demon Slayer - Mugen Train on Blu-ray on Blu-ray', 34.98);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Spirited Away', 1, 'The movie Spirited Away, DVD and Blu-ray combo', 29.95);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Cowboy Bebop', 1, 'Complete series of Cowboy Bebop on Blu-ray', 59.98);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Samurai Champloo', 1, 'Complete series of Samurai Champloo', 49.98);

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Traditional Shogi Game Set', 2, 'An elegant and practical Japanese chess set and comes with convenient drawers to store the Shogi playing pieces (10.6 x 9.8 x 1.8 inches)', 75.99);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Daruma Otoshi', 2, 'A high-quality wooden Japanese Daruma Otoshi game and comes with the hammer (9 x 1.8 inches)', 16.80);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Kendama', 2, 'Japanese catch and toss game, but enjoyed around the world. Our kendama toy measures 7 inch tall and is made of high-quality wood material', 14.97);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Portable and Magnetic Shogi Game Set', 2, 'The chess set is made of plastic material, portable, and durable with all game pieces included (9.75 x 9.75 x 0.9 inches)', 10.99);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Wood Go Game Set', 2, 'This Go board is made from high quality wood with storage drawers are included on both ends for storing the stones (12 x 1 x 12 inches)', 59.99);

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Metal Build Crossbone Gundam', 3, 'Crossbone Gundam X-0 Full Cloth, a protagonist in Mobile Suit Crossbone Gundam: Ghost (6.7 inch)', 320.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('MG Sazabi Version Ka', 3, '1/100 Scale colored plastic model kit comes with runner x 31, instruction manual, and stickers', 165.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Valtcan Prime Model Nipper', 3, 'Precision Model Kit Nippers with blade case, ultra sharp cutting edge and spring assistance (5 x 3 x 0.8 inches)', 30.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('HGCE Destiny Gundam', 3, '1/144 Scale colored plastic model kit comes with runner x 11, instruction manual, and stickers (2 x 6 x 4 inches)', 42.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('RG Sword Impulse Gundam', 3, 'Real grade 1/144 scale colored plastic model kit', 134.80);

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Glico Pocky',4,'Chocolate cream covered biscuit sticks with crunchy texture and smooth and creamy covering (9 bags/box)', 8.99);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('gimMe Roasted Seaweed',4,'A sea salt flavoured healthy on-the-go snack that is enjoyed by kids and adults (20 pack)', 15.99);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Japanese Kit Kat',4,'Contains random assortment of 16 mini Kit Kat candy bars', 19.99);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Japanese Snacks Assortment',4,'30pcs delicious snack selection of Japanese Dagashi', 18.99);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Amanoya Rice Crackers',4,'A tasty and crunchy Japanese rice crackers (3.45 ounce / pack, pack of 20)', 33.83);

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Campus Loose Leaf Paper Smooth Lined 7mm A4', 5, 'Campus loose leaf paper A4 size and horizontally ruled (7mm rule, 35 lines and 30 holes on each sheet)', 7.40);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Sarasa Clip - 0.5 Black', 5, '0.5 mm black ink pen', 2.30);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Mono Graph Mechanical Pencil 0.5mm', 5, 'A mechanical pencil with eraser attached at the top', 7.90);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Maruman Session Binder Pink B5 Size', 5, 'B5 (276 x 218 mm) binder features double W index for neat sorting. Comes with a clear pocket to store paper files', 7.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Campus notebook - Wide Dotted Rulled Line', 5, 'Printed in a dot grid style with horizontal lines running through them (size B5 with each page has 27 lines and 8mm horizontal rule)', 3.80);

INSERT INTO warehouse(warehouseName) VALUES ('Main warehouse');
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (1, 1, 5, 34.98);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (2, 1, 10, 34.98);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (3, 1, 3, 29.95);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (4, 1, 2, 59.98);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (5, 1, 6, 49.98);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (6, 1, 3, 75.99);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (7, 1, 4, 16.80);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (8, 1, 8, 14.97);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (9, 1, 3, 10.99);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (10, 1, 5, 59.99);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (11, 1, 9, 320.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (12, 1, 7, 165.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (13, 1, 8, 30.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (14, 1, 5, 42.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (15, 1, 6, 134.80);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (16, 1, 15, 8.99);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (17, 1, 12, 15.99);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (18, 1, 8, 19.99);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (19, 1, 3, 18.99);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (20, 1, 16, 33.83);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (21, 1, 10, 7.40);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (22, 1, 15, 2.30);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (23, 1, 9, 7.90);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (24, 1, 7, 7.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (25, 1, 12, 3.80);

INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Arnold', 'Anderson', 'a.anderson@gmail.com', '204-111-2222', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'arnold' , 'test');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Bobby', 'Brown', 'bobby.brown@hotmail.ca', '572-342-8911', '222 Bush Avenue', 'Boston', 'MA', '22222', 'United States', 'bobby' , 'bobby');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Candace', 'Cole', 'cole@charity.org', '333-444-5555', '333 Central Crescent', 'Chicago', 'IL', '33333', 'United States', 'candace' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Darren', 'Doe', 'oe@doe.com', '250-807-2222', '444 Dover Lane', 'Kelowna', 'BC', 'V1V 2X9', 'Canada', 'darren' , 'pw');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Elizabeth', 'Elliott', 'engel@uiowa.edu', '555-666-7777', '555 Everwood Street', 'Iowa City', 'IA', '52241', 'United States', 'beth' , 'test');

-- Order 1 can be shipped as have enough inventory
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (1, '2019-10-15 10:25:55', 144.95)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 1, 1, 34.98)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 1, 49.98)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 10, 1, 59.99);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-16 18:00:00', 47.97)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 17, 3, 15.99);

-- Order 3 cannot be shipped as do not have enough inventory for item 19
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (3, '2019-10-15 3:30:22', 110.75)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 23, 2, 7.90)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 19, 5, 18.99);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-17 05:45:11', 322.73)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 16, 4, 8.99)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 8, 3, 14.97)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 12, 1, 165.00)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 20, 2, 33.83)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 22, 4, 2.30);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (5, '2019-10-15 10:25:55', 371.38)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 11, 1, 320.00)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 18, 2, 19.99)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 25, 3, 3.80);

-- New SQL DDL for lab 8
UPDATE Product SET productImageURL = 'img/yourname.jpg' WHERE ProductId = 1;
UPDATE Product SET productImageURL = 'img/demonslayer.jpg' WHERE ProductId = 2;
UPDATE Product SET productImageURL = 'img/spiritedaway.jpg' WHERE ProductId = 3;
UPDATE Product SET productImageURL = 'img/cowboy.jpg' WHERE ProductId = 4;
UPDATE Product SET productImageURL = 'img/samurai.jpg' WHERE ProductId = 5;

UPDATE Product SET productImageURL = 'img/shogi.jpg' WHERE ProductId = 6;
UPDATE Product SET productImageURL = 'img/daruma.jpg' WHERE ProductId = 7;
UPDATE Product SET productImageURL = 'img/kendama.jpg' WHERE ProductId = 8;
UPDATE Product SET productImageURL = 'img/magnetic_shogi.jpg' WHERE ProductId = 9;
UPDATE Product SET productImageURL = 'img/go.jpg' WHERE ProductId = 10;

UPDATE Product SET productImageURL = 'img/crossbone.jpeg' WHERE ProductId = 11;
UPDATE Product SET productImageURL = 'img/mgsazabi.jpg' WHERE ProductId = 12;
UPDATE Product SET productImageURL = 'img/nipper.jpg' WHERE ProductId = 13;
UPDATE Product SET productImageURL = 'img/destiny.jpg' WHERE ProductId = 14;
UPDATE Product SET productImageURL = 'img/swordimpulse.jpg' WHERE ProductId = 15;

UPDATE Product SET productImageURL = 'img/pocky.jpg' WHERE ProductId = 16;
UPDATE Product SET productImageURL = 'img/seaweed.jpg' WHERE ProductId = 17;
UPDATE Product SET productImageURL = 'img/kitkat.jpg' WHERE ProductId = 18;
UPDATE Product SET productImageURL = 'img/assortmentsnacks.jpg' WHERE ProductId = 19;
UPDATE Product SET productImageURL = 'img/ricecrackers.jpg' WHERE ProductId = 20;

UPDATE Product SET productImageURL = 'img/looseleaf.jpeg' WHERE ProductId = 21;
UPDATE Product SET productImageURL = 'img/sarasa.jpeg' WHERE ProductId = 22;
UPDATE Product SET productImageURL = 'img/pencil.jpeg' WHERE ProductId = 23;
UPDATE Product SET productImageURL = 'img/binder.jpeg' WHERE ProductId = 24;
UPDATE Product SET productImageURL = 'img/notebook.jepg' WHERE ProductId = 25;

-- Loads image data for product 1
-- UPDATE Product SET productImage =  WHERE ProductId = 1;